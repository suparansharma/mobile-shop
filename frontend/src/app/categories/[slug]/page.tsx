...
export async function generateMetadata(
  ...
  const category = await fetchCategoryDetails(resolvedParams.slug);
  ...
  return {
    title: `${category.name} | Gadget & Parrk`,
...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {/* Header */}
      <div className="bg-orange-500 rounded-3xl p-8 sm:p-12 text-white mb-10 flex flex-col items-center justify-center text-center shadow-lg relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-orange-500 to-orange-600 opacity-90 z-0"></div>
        ...
        <div className="relative z-10">
          ...
          {category.description && (
            <p className="text-orange-900 max-w-2xl text-lg">{category.description}</p>
          )}
        </div>
      </div>
